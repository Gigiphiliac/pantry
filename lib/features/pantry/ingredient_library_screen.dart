...
const _tierLabels = {3: 'Per-Recipe', 2: 'Bulk Staples', 1: 'Always Available'};
      ...
      body: entries.when(
        ...
        data: (allEntries) {
          final filtered = _query.isEmpty
              ...
              : allEntries
                    .where(
                      (e) => e.ingredientName.toLowerCase().contains(
                        _query.toLowerCase(),
                      ),
                    )
                    .toList();
                    ...
                    : ListView(
                        children: [
                          if (tier3.isNotEmpty) ...[
                            ...
                            ...tier3.map((e) => _LibraryRow(entry: e)),
                          ],
                          if (tier2.isNotEmpty) ...[
                            _TierHeader(label: _tierLabels[2]!),
                            ...tier2.map((e) => _LibraryRow(entry: e)),
                          ],
                          if (tier1.isNotEmpty) ...[
                            _TierHeader(label: _tierLabels[1]!),
                            ...tier1.map((e) => _LibraryRow(entry: e)),
                          ],
                        ],
                      ),
              ),
...
class _LibraryRow extends ConsumerWidget {
  ...
  const _LibraryRow({required this.entry});
  ...
  Future<void> _showEditSheet(BuildContext context, WidgetRef ref) async {
    await showModalBottomSheet(
      ...
      builder: (ctx) => _EditLibraryItemSheet(entry: entry),
    );
  }
}

class _EditLibraryItemSheet extends ConsumerStatefulWidget {
  final PantryEntry entry;
  const _EditLibraryItemSheet({required this.entry});

  @override
  ConsumerState<_EditLibraryItemSheet> createState() =>
      _EditLibraryItemSheetState();
}

class _EditLibraryItemSheetState extends ConsumerState<_EditLibraryItemSheet> {
  ...
  void initState() {
    ...
    _unitController = TextEditingController(
      text: widget.entry.preferredUnit ?? '',
    );
  }
      ...
      child: Column(
        ...
        children: [
          ...
          SegmentedButton<int>(
            ...
            onSelectionChanged: (s) => setState(() => _selectedTier = s.first),
          ),
          const SizedBox(height: 6),
          Text(
            const {
