...
class PantryOps {
  ...
  Future<void> addStock(int ingredientId, int categoryId,
      {double? qty, String? unit, String? notes}) async {
    await db.into(db.pantryStock).insertOnConflictUpdate(
  ...
  Future<void> updateStockQty(int stockId, double? qty) async {
    await (db.update(db.pantryStock)
        ...
        .write(PantryStockCompanion(onHandQty: Value(qty)));
  }
  ...
  Future<void> updateStockUnit(int stockId, String? unit) async {
    await (db.update(db.pantryStock)
          ..where((t) => t.id.equals(stockId)))
        .write(PantryStockCompanion(onHandUnit: Value(unit)));
  }
  ...
  Future<void> updateStockNotes(int stockId, String? notes) async {
    await (db.update(db.pantryStock)
          ..where((t) => t.id.equals(stockId)))
        .write(PantryStockCompanion(notes: Value(notes)));
  }
  ...
  Future<void> moveStockItem(int stockId, int newCategoryId) async {
    await (db.update(db.pantryStock)
          ..where((t) => t.id.equals(stockId)))
        .write(PantryStockCompanion(categoryId: Value(newCategoryId)));
  }
  ...
  Future<void> deleteStockItem(int stockId) async {
    await (db.delete(db.pantryStock)
          ..where((t) => t.id.equals(stockId)))
        .go();
  }
  ...
  Future<int> createCategory(String name) async {
    final count = await (db.select(db.pantryStockCategories).get())
        .then((l) => l.length);
    return db.into(db.pantryStockCategories).insert(
  ...
  Future<void> deleteCategory(int categoryId) async {
    final stockCount = await (db.select(db.pantryStock)
    ...
    await (db.delete(db.pantryStockCategories)
          ..where((t) => t.id.equals(categoryId)))
        .go();
  }
  ...
  Future<void> moveCategoryItems(int fromCategoryId, int toCategoryId) async {
    ...
    await (db.delete(db.pantryStockCategories)
          ..where((t) => t.id.equals(fromCategoryId)))
        .go();
  }
}
